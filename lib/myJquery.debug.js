...
$(function () {
	...
	function myPhotos() {
		...
		function buildPhotosWall (index) {
			var $albumWall = $('.album_wall_area'),lis = $albumWall.find('li> div'),data = datas[index],rows = data['photos'],length = rows.length,url = data['url'],wallWidth = $albumWall.width()
			...
			while(widthfFlag[0] || widthfFlag[1] || widthfFlag[2]) {
				...
				var item = lis.eq(flag), indexFlag = randArray[ randArray.length - 1 ];
				randArray.pop();
				item.append('<div class="img_box"><img src="'+ url + rows[indexFlag]['text'] +'" ></div>');
