...
$(function () {
	...
	function myArticle () {
		var bodyWidth = $.getValues().bodyWidth,bodyHeight = $.getValues().bodyHeight,animateTime = $.getValues().animateTime;
		var datas = data_myArticle,$content = $('.content'), $myArticle = $('.box_myArticle'),moduleHeight = $myArticle.height(),$btns = $myArticle.find('div.article_menu_box'),$walls = $myArticle.find('.wall_paper')
			,$mainArea = $myArticle.find('.article_main_area'), $mainEssay = $('.main_area_essay');
		$content.height(moduleHeight);
		/* 导航按钮点击事件 */
		$content.click(function (e) {
			var $this = $mainEssay.find('.active');
			if($this.length > 0){
		...
		(function () {
			...
			$previous.click(function () {
				var $this = $mainEssay.find('>.current') ;
