...
$(function () {
	...
	function myArticle () {
		var bodyWidth = $.getValues().bodyWidth,bodyHeight = $.getValues().bodyHeight,animateTime = $.getValues().animateTime;
		var $content = $('.content'), $myArticle = $('.box_myArticle'),moduleHeight = $myArticle.height(),$btns = $myArticle.find('div.article_menu_box'),$walls = $myArticle.find('.wall_paper')
			,$mainArea = $myArticle.find('.article_main_area'), $mainEssaty = $('.main_area_essay');
		$content.height(moduleHeight);
		/* 导航按钮点击事件 */
		$content.click(function (e) {
			var $this = $mainEssaty.find('.active');
			if($this.length > 0){
		...
		$mainEssaty.find('.essay_item').click(function (e) {
			stopBubble(e);
		})
		$mainEssaty.find('.essay_title ').click(function (e) {
			
			var $this = $(this), $parent = $this.parent(), $list = $parent.parent(),$active = $mainEssaty.find('.active'),top = $this.offset().top;
			if($active.length == 0){
