...
$(function () {
	...
	function myPhotos() {
		$('.needtooltip').tooltip();
		var bodyWidth = $.getValues().bodyWidth,bodyHeight = $.getValues().bodyHeight;
		var $myPhotos = $('.box_myPhotos'),datas = data_myPhotos;
		$('#myPhoto_adjust').click(function () {
			$myPhotos.toggleClass('neight_style');
		});
		buildPhotos(datas);
		function buildPhotos (_photos) {
			var photos = _photos, length = photos.length,html = [],$photosArea = $('.photos_area');
			for(var i = 0; i < length; i ++ ) {
				var item = photos[i], title = item.title,url = item.url;
				html.push('<div class="photo_area"><div class="photo_box3"><div class="photo_content"></div></div><div class="photo_box2"><div class="photo_content"></div></div><div class="photo_box"><div class="photo_content">');
		...
		function getPhotoFream (_this) {
			var $this = _this,offset = $this.parent().offset(),left = offset.left,top = offset.top;
