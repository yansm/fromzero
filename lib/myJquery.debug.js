...
$(function () {
	...
	function myPhotos() {
		...
		function buildPhotosNormal (index) {
			var $album = $('.album_normal'),$photosList = $album.find('.photos_list'),data = datas[index],rows = data['photos'],length = rows.length,url = data['url'],html = [];
			$photosList.html('').removeAttr('style');
			for(var i = 0; i < length; i ++) {
				var item = rows[i],text = item['text'];
				html.push('<li><img src="'+ url + text +'" ></li>');
			}
			$photosList.html(html.join('')).parents('.left_area').jScrollPane();
